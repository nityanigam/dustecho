...
#modify geometry to allow for cylindrically symmetric geometries
#TO DO: change input lightcurve, change geometry, change dust distribution, change gas distribution

# adjustable parameters
nH0 = 1                # [cm^-3] H number density
tmax = 300.     # [s], duration of the source (not variable lightcurve!)
LUV = 3e47      # [erg/s]  # source luminosity (flat)

# fixed parameters
tmin = 0.
n0_over_nH = 1.45e-15    # dust number density over H number density
lam0 = 2.       # [um] critical wavelength for Qabs_lambda, Qabs absorption efficiency
#thej = 4*pi/180     # [rad] jet opening angle -- we don't need to account for jet
thej = 4*pi
p = 2.2         # electron PL index => spectrum L_nu ~ nu^{(1-p)/2} power law
...
Na = 30 #number of grain sizes?
aarr = np.logspace(log10(amin), log10(amax), Na) #logarithmic because of power law distribution (eq10)
...
Nnu = 40
nuarr = np.logspace(log10(numin), log10(numax), Nnu)    # frequency bins (power law SED)
nu_ratio = nuarr[1]/nuarr[0]

# jet emission time [dust local frame]
Nt = 20     # this is the dimension we interpolate over
tarr = np.linspace(tmin, tmax, Nt, endpoint=False) #duration of jet emission time
dt = tarr[1] - tarr[0]
tarr += dt/2.
...
def func_Lnu(t, nu):     # source spectrum and light curve (can change this spectrum)
    if t < tmax:
        return LnuUVmax*(nu/nuUVmax)**((1 - p)/2)  # spectrum L_nu ~ nu^{(1-p)/2}
    return 0. # can probably adjust this to take in an actual light curve, rather than flat for fixed duration


def func_nH(r):      # gas density profile (r in pc) -- currently H number density
    return nH0


def func_Qabs(nu, a):      # absorption efficiency for grain size a [um]
    lam = const.C_LIGHT/nu * 1e4    # wavelength in um
    return 1./(1 + (lam/lam0)**2 / a) #eq11 in paper


def func_T(qdot_h, aum):    # solve the heating and cooling balance for T -- eq19 in paper
...
for i in range(Nr_fine):
    ...
    NH += 4*pi*r*r*dr*const.pc2cm**3 * func_nH(r) #integrating H atoms in log space
    NHarr_fine[i] = NH
...
Nnu_fine = int(Nnu*5)
nu_ion_min, nu_ion_max = 13.6/(const.erg2eV*const.H_PLANCK), 100/(const.erg2eV*const.H_PLANCK) #ionizing freqs
...
for i in range(Nt_fine):
    ...
    for j in range(Nnu):
        nu = nuarr_fine[j]
        dnu = nu * (sqrt(nu_ratio_fine) - 1/sqrt(nu_ratio_fine)) #same integration
        Lion += dnu * func_Lnu(t, nu)/(const.H_PLANCK*nu) #integrating for photons
    Nion += Lion*dt_fine
    Nionarr_fine[i] = Nion
...
def calculate_all_T(t, i_t, rion, Tarr, asubarr, taudarr):
    for j in range(Nr):
        ...
        for k in range(Na):
            ...
            T = func_T(qdot_h, a)
            Tsub = 2.33e3 * (1 - 0.033*log(t/100./a)) #eq21 in paper
...
def calculate_taudarr(i_t, asubarr, taudarr):
    for m in range(Nnu):
        ...
        tau_pre_factor = 2*sqrt(2)*pi*1e-8*n0_over_nH*lam0/lam #eq24 from paper
        taud = 0.
        for j in range(Nr):
            r = rarr[j]
            ...
            taud += tau_pre_factor * dr * const.pc2cm * func_nH(r) \
                    * (atan(sqrt(0.5*xmax)) - atan(sqrt(0.5*xsub))) #eq24
            taudarr[m, j] = taud
    return


tol = 0.01  # tolerance for asubarr
for i in range(Nt): #actually solving the iterative system
    t = tarr[i]
    Nion = Nion_t_intp(t)
