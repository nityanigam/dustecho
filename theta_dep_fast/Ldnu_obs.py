    ...
    for j in range(Nr):
        for l in range(Nthe):
            ...
            while a <= amax:
                ...
                if Tarr[i, j, k, l] < 100:  # ignore extremely cold dust
                    a *= a_ratio
                    continue
                j_integ += da * a**-0.5 /(a + (lamobs/lam0)**2) \
                           / (exp(h_nu_over_k/Tarr[i, j, k, l]) - 1)
                a *= a_ratio
    ...
    for j in range(Nr):
        ...
        for k in range(Nthe):
            ...
            while mu < mumax:
                ...
                jdnu = jdnu_intp(t, j, jdnuarr, tarr, k)
                mu_integ += dmu * jdnu * 2 * tphimax
                mu_integ_xcentr += dmu * r * sqrt(1-mu*mu) * jdnu * 2 * sin(tphimax)
                mu += dmu
            # print(mu_integ)
            Ldnu_the += np.sin(the) * Dthe * mu_integ
            Ldnu_xcentr_the += np.sin(the) * Dthe * mu_integ_xcentr
        Ldnu += 2*pi*dr*r*r * const.pc2cm**3 * Ldnu_the
        Ldnu_xcentr += 2*pi*dr*r*r * const.pc2cm**3 * Ldnu_xcentr_the
    Ldnuarr[i_tobs] = Ldnu
