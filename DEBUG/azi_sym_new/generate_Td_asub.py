...
tarr_fine += dt_fine/2.
Nionarr_fine = np.zeros((Nt_fine, Nthe_fine), dtype=float)   # cumulative number of ionizing photons
Nnu_fine = int(Nnu*5)
...
for i in range(Nt):
    print(np.mean(Nionarr_fine[i*20,:]))
    print(np.sum(Nionarr_fine[i*20,:]))
    print(i)
        ...
        for j in range(Nr):
            ...
            for k in range(Na):
                ...
                if T > Tsub:    # grains in this size bin have already evaporated
                    ...
                    Tarr[i_t, j, k, l] = 0.   # no emission from this size bin
                else:   # grains survive
                    Tarr[i_t, j, k, l] = T
            if i_t == 0:
                        ...
                        for k in range(Na):
                            if k == 0:
                                ...
                                if i%10 == 0 and j%10 == 0 and k%10 == 0:
                                    ...
                                    print("radius index: " + str(j))
                                    print("a index: " + str(k))
                                    print(np.mean(Tarr[i,j,k,:]))
                            else:
                                f.write('\t%.8e' % Tarr[i, j, k, l])
                                if i%10 == 0 and j%10 == 0 and k%10 == 0:
                                    print("time index: " + str(i))
